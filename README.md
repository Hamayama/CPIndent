...
- また、block no.のチェックボックスにチェックを入れると、  
  `#` のところに番号を表示します。  
  `#if` と `#endif` の対応を確認したい場合等に利用ください。
...
## 注意事項
1. Cプリプロセッサの命令のみインデントされます。

2. インデントの単位は、半角スペース4個に固定です。

3. 半角スペースを追加するのみであるため、元々変なインデントになっている場合には、  
   うまく整形できません。
